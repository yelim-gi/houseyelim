...
v44 수정:
- 매입가격/총 투자원금은 매출이나 순이익이 들어온다고 줄어드는 값이 아닙니다. 남은 재고 매입가와 누적 매입/투자원금은 분리해서 봐야 합니다.
- 랜덤스쿱 추천안 상품교체 기능 보강
- 수동박스/랜덤스쿱에 '원하는 캐릭터만 구성' 체크 옵션 추가
- 주문상품보기에서 총 도매가합/총 소비자가합 표시
- 주문접수/출고완료 건 모두 주문상품보기 가능
- 주문상품보기 후 상품 교체/삭제/추가 가능
- 주문상품 교체/삭제/추가 시 재고 자동 보정


v46 안전 수정:
- v44 기능을 기준으로 기존 기능 유지
- 총 투자원금(고정) 입력값만 최소 추가
- 대시보드에 총 투자원금(고정), 현재 남은 재고 매입가, 현재 남은 재고 소비자가 분리 표시
- 코드 파일 교체는 Supabase의 기존 재고/주문 데이터를 초기화하지 않음


v47 수정:
- 총 투자원금은 직접 입력이 아니라 전체 재고 도매가 총합 + 재료비 총합으로 자동 계산
- 현재 남은 재고 매입가는 현재 재고 수량 기준으로 별도 계산
- 매출/순이익은 총 투자원금에서 차감하지 않음
- 수동박스/랜덤스쿱 '원하는 캐릭터만 구성' 체크 옵션 보강
- 주문상품보기에서 총 도매가합/총 소비자가합 표시
- 출고완료건도 주문상품보기 가능
- 주문상품 교체/삭제/추가 가능 및 재고 자동 보정
- 랜덤스쿱 추천안 상품교체 보강
